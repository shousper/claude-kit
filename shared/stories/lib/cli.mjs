// shared/stories/lib/cli.mjs — arg parsing + subcommand dispatch.
//
...
import { CliError, EXIT, failureOutput, nowISO, run, todayISO, writeFileAtomic, GITIGNORE_BLOCK, LEGACY_CONFIG, STATE_DIR, configPath, hasLegacyMarker, hasMarker } from "./util.mjs";
 ...
 * .git from both. A project still on the pre-split layout is accepted only
 * by the commands that can act on it (doctor migrates, context explains).
 */
export function findRoot(cwd, exec = run, { allowLegacy = false } = {}) {
  ...
  if (hasLegacyMarker(root)) {
    if (allowLegacy) return root;
    throw new CliError(`legacy layout (${LEGACY_CONFIG}) — run story doctor once to migrate it to ${STATE_DIR}/`);
  }
  throw new CliError(`no ${STATE_DIR}/config.json found — run stories:setup (or \`story init\`) first`);
}

/** findRoot + loadConfig — the read-only pair almost every command starts with. */
function resolveProject(ctx, opts) {
  const root = findRoot(ctx.cwd, ctx.exec, opts);
  return { root, config: board.loadConfig(root) };
}

/** Read a file path flag (--config/--body-file/--plan-file), relative to ctx.cwd. */
function readFlagFile(ctx, flag) {
  return readFileSync(resolve(ctx.cwd, ctx.flags[flag]), "utf8");
}

/** ctx.positionals[0], or throw CliError(usage) when it is missing. */
function requireId(ctx, usage) {
  const [id] = ctx.positionals;
  if (!id) throw new CliError(usage);
  return id;
}

/** The harness adapter's session id (STORY_SESSION_ID); empty strings count as unset. */
function sessionFromEnv(env) {
  const v = env.STORY_SESSION_ID;
...
async function cmdInit(ctx) {
  ...
  if (existsSync(cfgPath)) throw new CliError(`${STATE_DIR}/config.json already exists`);
  if (hasLegacyMarker(root)) {
    throw new CliError(`legacy layout (${LEGACY_CONFIG}) — run story doctor to migrate it instead of re-initializing`);
  }
  ...
  mkdirSync(board.archiveDir(root, config), { recursive: true });
...
function initConfigFromFlags(ctx) {
  const merge = ctx.flags.merge ?? board.CONFIG_DEFAULTS.merge;
  if (!board.MERGE_MODES.includes(merge)) {
    throw new CliError(`--merge must be one of ${board.MERGE_MODES.join("|")}, got '${merge}'`);
  }
  return {
    ...board.CONFIG_DEFAULTS,
    merge,
    baseBranch: ctx.flags["base-branch"] ?? board.CONFIG_DEFAULTS.baseBranch,
...
function initConfigFromFile(ctx) {
  ...
  let config;
  try {
    config = JSON.parse(readFlagFile(ctx, "config"));
  } catch (err) {
  ...
  if (!board.MERGE_MODES.includes(config.merge)) {
    throw new CliError(`config.merge must be one of ${board.MERGE_MODES.join("|")}, got '${config.merge}'`);
  }
...
async function cmdCreate(ctx) {
  ...
  const story = await withLock(root, LOCK.BOARD, () => {
    ...
    for (const g of s.gates ?? []) {
      if (!(config.gates ?? {})[g]) {
        throw new CliError(`unknown gate '${g}' — define it in ${STATE_DIR}/config.json`);
      }
    }
...
async function warnTouchesExpansion(ctx, root, config, stories, diffs) {
  const active = stories.filter(board.isActive);
  for (const a of active) {
    ...
    for (const b of active) {
      ...
      await withLock(root, LOCK.BOARD, () => {
        const fresh = board.findStory(root, config, a.id);
        if (fresh.body.includes(marker)) return; // raced another warner
        board.saveStory(root, config, {
          ...fresh,
          updated: todayISO(),
          body: board.appendNote(fresh.body, `${marker} (${overlap.join(", ")}) — coordinate or split`),
        });
      });
    }
  }
...
/** The claim-safe workable set in its public shape (feedback items first, priority defaulted). */
function readyEntries(stories, diffs) {
  return board.computeReady(stories, { diffs }).map((s) => ({
    id: s.id,
    title: s.title,
...
async function cmdReady(ctx) {
  ...
  const ready = readyEntries(stories, diffs);
  output(
    ctx,
    ready,
    ready.length
      ? ready.map((s) => `${s.id}  ${s.priority}  ${s.feedback ? "[feedback] " : ""}${s.title}`)
      : ["(nothing ready)"],
  );
}

async function cmdClaim(ctx) {
  const id = requireId(ctx, "usage: story claim <id>");
  const { root, config } = resolveProject(ctx);
  const session = ctx.flags.session ?? sessionFromEnv(ctx.env) ?? randomUUID();
  ...
  const worktree = await withLock(root, LOCK.BOARD, () => {
    const stories = board.loadStories(root, config, { includeArchive: true });
    ...
    board.assertTransition(story.status, board.STATUS.IN_PROGRESS);
    const preexisting = existsSync(worktrees.worktreePath(root, id));
    const path = worktrees.createWorktree(root, id, { exec: ctx.exec, base: config.baseBranch });
    const next = { ...story, status: board.STATUS.IN_PROGRESS, claim: { session, lease: nowISO() }, updated: todayISO() };
...
async function mutateStory(ctx, id, fn) {
  const { root, config } = resolveProject(ctx);
  return board.mutateStory(root, config, id, fn, { heartbeat: true });
}

const SCALAR_UPDATES = { title: "title", type: "type", epic: "epic", "discovered-from": "discovered_from" };

async function cmdUpdate(ctx) {
  const id = requireId(ctx, "usage: story update <id> --<field> <value> …");
  const story = await mutateStory(ctx, id, (s) => {
    const wasInProgress = s.status === board.STATUS.IN_PROGRESS;
    if (ctx.flags.status !== undefined) {
      ...
      if (s.status !== board.STATUS.IN_PROGRESS) delete s.claim;
    }
    ...
    if (typeof ctx.flags["plan-file"] === "string") {
      const plan = readFlagFile(ctx, "plan-file");
      s.body = board.setSection(s.body, board.SECTIONS.PLAN, plan.trim());
    }
  });
...
async function cmdNote(ctx) {
  const id = requireId(ctx, "usage: story note <id> --body <text>");
  if (typeof ctx.flags.body !== "string") throw new CliError("usage: story note <id> --body <text>");
  await mutateStory(ctx, id, (s) => {
    s.body = board.appendNote(s.body, ctx.flags.body);
  });
  output(ctx, { id, noted: true }, [`noted on ${id}`]);
}

async function cmdPark(ctx) {
  const id = requireId(ctx, "usage: story park <id> --question <text>");
  if (typeof ctx.flags.question !== "string") throw new CliError("usage: story park <id> --question <text>");
  await mutateStory(ctx, id, (s) => {
    ...
    return board.parkStory(s, ctx.flags.question); // park-and-continue: the worker moves on, the lease must not go stale
  });
  output(ctx, { id, status: board.STATUS.BLOCKED }, [`parked ${id} — question recorded`]);
}
...
async function cmdRecord(ctx) {
  ...
  const id = requireId(ctx, "usage: story record <id> --gate <name> --verdict pass|fail [--evidence <path>]");
  if (typeof gate !== "string" || typeof verdict !== "string") {
    throw new CliError("usage: story record <id> --gate <name> --verdict pass|fail [--evidence <path>]");
  }
  const { root, config } = resolveProject(ctx);
  board.findStory(root, config, id); // must exist
  const def = (config.gates ?? {})[gate];
  if (!def || def.kind !== "review") {
    throw new CliError(`'${gate}' is not a review gate in ${STATE_DIR}/config.json`);
  }
...
// Minimum word count for an implementation plan to count as "on record" by
// the 0c guard below — terse plans pass, an empty/placeholder section does not.
const PLAN_MIN_WORDS = 10;
...
async function cmdDone(ctx) {
  const id = requireId(ctx, "usage: story done <id>");
  const { root, config } = resolveProject(ctx);
  const base = config.baseBranch;
  const story = board.findStory(root, config, id);
  if (story.status === board.STATUS.DONE) {
    ...
    output(ctx, { id, status: board.STATUS.DONE, already: true }, [`${id} already done — nothing to do`]);
    return;
  }
  if (story.status !== board.STATUS.IN_PROGRESS) {
  ...
  if (status.code !== 0) {
    throw new CliError(`git status failed in ${worktree}: ${failureOutput(status)}`);
  }
  ...
  if (ahead.code !== 0) {
    throw new CliError(`cannot count commits on ${worktrees.branchName(id)}: ${failureOutput(ahead)}`);
  }
  ...
  const plan = board.getSection(story.body, board.SECTIONS.PLAN) ?? "";
  if (plan.split(/\s+/).filter(Boolean).length < PLAN_MIN_WORDS && ctx.flags["allow-unplanned"] !== true) {
    throw new CliError(
  ...
  const mode = config.merge;

  if (mode === "pr") {
    // Dynamic import (ratified cycle rule): cli.mjs must not import github.mjs statically.
    const { integratePrMode } = await import("./github.mjs");
    const res = await integratePrMode(root, story, { exec: ctx.exec, diff });
    output(ctx, { id, status: board.STATUS.IN_REVIEW, pr: res.number, created: res.created, evidence }, [
  ...
  if (mode === "self") {
    ...
    if (conflict && worktrees.safeToDiscardOnConflict(root, id, { exec: ctx.exec, base })) {
      ...
      await board.mutateStory(root, config, id, (s) => {
        s.body = board.appendNote(
          s.body,
          `integration conflict with ${base}; merge ${base} into ${worktrees.branchName(id)} in the worktree, resolve, then re-run \`story done ${id}\``,
        );
      }, { heartbeat: true });
      throw new CliError(
    ...
    await board.mutateStory(root, config, id, (s) => board.closeStory(worktrees.reconcileTouches(s, diff)));
    await commitBoard(ctx, root, config, id);
    output(ctx, { id, status: board.STATUS.DONE, merged: true, evidence }, [`${id} done — merged into ${base}`]);
    return;
  }

  // local mode: hand off to the human, hold the reconciled touches.
  await board.mutateStory(root, config, id, (s) => board.releaseClaim(worktrees.reconcileTouches(s, diff), board.STATUS.IN_REVIEW));
  output(ctx, { id, status: board.STATUS.IN_REVIEW, evidence }, [
...
async function commitBoard(ctx, root, config, id) {
  await withLock(root, LOCK.MERGE, () => {
    ...
    if (commit.code !== 0) {
      ctx.stderr.write(`warning: board commit failed (edits left for the next close): ${failureOutput(commit)}\n`);
    }
  });
}
...
async function cmdDoctor(ctx) {
  const root = findRoot(ctx.cwd, ctx.exec, { allowLegacy: true });
  // The one command that runs on the pre-split layout: it moves the project
  // across first, then checks the board like any other run.
  const migration = await withLock(root, LOCK.BOARD, () => migrateLayout(root));
  const config = board.loadConfig(root);
  const report = await withLock(root, LOCK.BOARD, () =>
    runDoctor(root, config, { fix: ctx.flags.fix === true, exec: ctx.exec }),
  );
  if (migration) report.fixed.unshift(migration);
  const hard = report.issues.some((i) => i.hard === true);
  if (!(ctx.flags.quiet === true && report.ok && !migration)) {
    const lines = migration
      ? [
          `layout-migrated: ${migration.moved.length} file(s) moved to ${STATE_DIR}/ (.gitignore ${migration.gitignore})`,
          ...(migration.personas ? [`note: ${migration.personas}`] : []),
        ]
      : [];
    output(
      ctx,
...
async function cmdArchive(ctx) {
  const { root, config } = resolveProject(ctx);
  const archived = await withLock(root, LOCK.BOARD, () => {
    const done = board.loadStories(root, config).filter((s) => s.status === board.STATUS.DONE);
    const dir = board.archiveDir(root, config);
...
const STATUS_COLUMN_WIDTH = Math.max(...board.STATUSES.map((s) => s.length));
const listLine = (s) => `${s.id}  ${String(s.status).padEnd(STATUS_COLUMN_WIDTH)}  ${s.priority}  ${s.title}`;
...
async function cmdShow(ctx) {
  const id = requireId(ctx, "usage: story show <id>");
  const { root, config } = resolveProject(ctx);
  const s = board.findStory(root, config, id, { includeArchive: true });
  const pr = board.prNumber(s);
...
async function cmdLoop(ctx) {
  ...
  const envSession = sessionFromEnv(ctx.env);
  if (flags.session === undefined && envSession) flags.session = envSession;
  const result = await runLoopCommand({ positionals: ctx.positionals, flags }, { root });
  if (ctx.positionals[0] === "tick") return emitTick(ctx, result);
  output(ctx, result, [loopHumanLine(ctx.positionals[0], result)]);
}

// Adapter contract for the turn-end hook on both harnesses: exit 2 = block
// (stdout: status line, blank line, reason), exit 0 = allow (stdout: summary
// or nothing). --json prints the same fields structurally, same exit codes.
function emitTick(ctx, r) {
  const block = r.decision === "block";
  if (ctx.json) {
    ctx.stdout.write(`${JSON.stringify(r)}\n`);
  } else if (block) {
    ctx.stdout.write(`${r.status ?? ""}\n\n${r.reason}\n`);
  } else if (r.summary) {
    ctx.stdout.write(`${r.summary}\n`);
  }
  return block ? EXIT.DENY : EXIT.OK;
}

function loopHumanLine(sub, result) {
  if (sub === "start") return `story loop started - goal: ${result.goal} (stall budget ${result.max_stalls})`;
  if (sub === "status") {
    if (Array.isArray(result.loops)) {
      return result.loops.length > 0
        ? result.loops
            .map((l) => `${l.session_id}: iteration ${l.iteration} · stalls ${l.stalls}/${l.max_stalls} - goal: ${l.goal}`)
            .join("\n")
        : "no active loops";
    }
    return result.active
      ? `loop active - iteration ${result.iteration} · stalls ${result.stalls}/${result.max_stalls} - goal: ${result.goal}`
      : "no active loop";
  }
...
// The session-start block both harness adapters inject. Rules come from the
// using-stories skill beside this library (resolved through the module's real
// path, so it works through the plugins' lib symlinks); the board snapshot
// comes from the same readers `story ready` and `story loop status` use.
const RULES_URL = new URL("../skills/using-stories/SKILL.md", import.meta.url);
const RULES_FALLBACK = "(stories:using-stories skill body unavailable - load the stories:using-stories skill)";

function legacyNotice() {
  return `This project is on the legacy state layout (${dirname(LEGACY_CONFIG)}/): run \`story doctor\` once to migrate it to ${STATE_DIR}/ before using any other story command.`;
}
...
function renderContext(d) {
  const lines = [
    ...
    `The story CLI is at: ${d.cli} (use this absolute path whenever a bare \`story\` is not on PATH).`,
    "",
  ];
  if (d.legacy) lines.push(d.notice, "");
  lines.push(
  ...
  if (!d.legacy) {
    lines.push(
      ...
      JSON.stringify(d.ready),
      "",
      "**Active loop (story loop status --json):**",
      JSON.stringify(d.loop),
    );
  }
...
async function cmdContext(ctx) {
  const root = findRoot(ctx.cwd, ctx.exec, { allowLegacy: true });
  const cli = ctx.env.STORY_BIN || process.argv[1] || "story";
  let rules;
  try {
    rules = readFileSync(RULES_URL, "utf8");
  } catch {
    rules = RULES_FALLBACK;
  }
  const legacy = !hasMarker(root);
  let ready = null;
  let loop = null;
  if (!legacy) {
    let config;
    try {
      config = board.loadConfig(root);
      const stories = board.loadStories(root, config, { includeArchive: true });
      ready = readyEntries(stories, worktrees.activeDiffs(root, config, stories, ctx.exec));
    } catch (err) {
      ready = { error: `story ready failed - run story doctor (${err.message})` };
      loop = { active: false };
    }
    if (config) {
      try {
        loop = await runLoopCommand({ positionals: ["status"], flags: {} }, { root });
      } catch {
        loop = { active: false };
      }
    }
  }
  const data = { cli, rules, ready, loop, ...(legacy ? { legacy: true, notice: legacyNotice() } : {}) };
  output(ctx, data, [renderContext(data)]);
}

// ---------------------------------------------------------------- guard

// Adapter contract: exit 0 = allow (no output), exit 2 = deny (reason on stdout),
// exit 1 = CLI failure (adapters treat it as allow: a broken CLI must never lock
// every edit in the project). --json prints {allow, reason?} with the same codes.
async function cmdGuard(ctx) {
  const tool = ctx.flags.tool;
  if (typeof tool !== "string" || !tool) {
    throw new CliError("usage: story guard --tool <name> [--path <file>] [--session <id>]");
  }
  const { root, config } = resolveProject(ctx);
  const session = typeof ctx.flags.session === "string" ? ctx.flags.session : sessionFromEnv(ctx.env);
  const verdict = classifyGuard(root, config, { tool, path: ctx.flags.path, session });
  if (verdict.allow) {
    if (ctx.json) output(ctx, verdict, []);
    return EXIT.OK;
  }
  output(ctx, verdict, [verdict.reason]);
  return EXIT.DENY;
}
...
export async function main(argv, io = {}) {
  ...
  try {
    ...
    return (await handler(ctx)) ?? EXIT.OK;
  } catch (err) {
