// shared/stories/lib/worktrees.mjs — story worktree + integration lifecycle.
// Every git call goes through the injectable exec (util.run signature).
import { existsSync, mkdirSync, realpathSync, rmSync } from "node:fs";
import { join, resolve, sep } from "node:path";
import { assertValidId, CONFIG_DEFAULTS, isActive } from "./board.mjs";
import { LOCK, withLock } from "./locks.mjs";
import { CliError, run, runOk, WORKTREES_DIR } from "./util.mjs";
...
export function worktreePath(root, id) {
  assertValidId(id);
  return join(worktreesRoot(root), id);
}
 ...
 */
export function createWorktree(root, id, { exec = run, base = CONFIG_DEFAULTS.baseBranch } = {}) {
 ...
 */
export function actualDiff(root, id, { exec = run, base = CONFIG_DEFAULTS.baseBranch } = {}) {
...
export function activeDiffs(root, config, stories, exec = run, exceptId = null) {
  ...
  for (const s of stories) {
    ...
    diffs.set(s.id, actualDiff(root, s.id, { exec, base: config.baseBranch }));
  }
  return diffs;
}

// Merge-commit subject prefix shared by the writer (integrateSelf) and the
// reader (isMergedSelf's --grep) so the two spellings cannot drift.
const MERGE_SUBJECT_PREFIX = "story ";

export function mergeCommitMessage(id, title) {
  return `${MERGE_SUBJECT_PREFIX}${id}: ${title}`;
}

function mergeCommitGrep(id) {
  return `^${MERGE_SUBJECT_PREFIX}${id}:`;
}
 ...
 */
export async function integrateSelf(root, story, { exec = run, base = CONFIG_DEFAULTS.baseBranch, lock = withLock } = {}) {
  return lock(root, LOCK.MERGE, () => {
    ...
    const merge = exec(
      "git",
      ["merge", "--no-ff", branchName(story.id), "-m", mergeCommitMessage(story.id, story.title)],
      { cwd: root },
    );
    if (merge.code !== 0) {
 ...
 */
export function isMergedSelf(root, id, { exec = run, base = CONFIG_DEFAULTS.baseBranch } = {}) {
  assertValidId(id);
  const r = exec("git", ["log", base, "-1", "--format=%H", "--grep", mergeCommitGrep(id)], { cwd: root });
 ...
 * trusting the grep.
 */
export function safeToDiscardOnConflict(root, id, { exec = run, base = CONFIG_DEFAULTS.baseBranch } = {}) {
...
export function isMergedLocal(root, id, { exec = run, base = CONFIG_DEFAULTS.baseBranch } = {}) {
...
export function teardown(root, id, { exec = run } = {}) {
  ...
  if (existsSync(path)) {
    ...
    if (r.code !== 0) {
      const base = realpathSync(worktreesRoot(root)) + sep;
