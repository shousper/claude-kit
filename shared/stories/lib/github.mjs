// shared/stories/lib/github.mjs — PR mode (merge: "pr"). Design §9.
//
...
// (loadConfig, plus every other board primitive via the `board` namespace
// import: closeStory, parkStory, releaseClaim, appendNote, appendQuestion,
// mutateStory, readBodySection) live in board.mjs — never imported from
// cli.mjs, which would violate the module boundary. This module is imported
// only DYNAMICALLY (cli.mjs inside handlers; loop.mjs inside tick), so
// nothing here closes a static cycle.
import { CliError, execError, failureOutput, readJson, run as defaultRun, sweepStatePath, writeJsonAtomic, WORKTREES_DIR } from "./util.mjs";
import { LOCK, withLock } from "./locks.mjs";
import * as board from "./board.mjs";
import { loadConfig, STATUS } from "./board.mjs";
...
// GitHub API enum values (gh pr view/list --json) — named so every
// comparison site spells them identically.
const PR_STATE = Object.freeze({ OPEN: "OPEN", MERGED: "MERGED", CLOSED: "CLOSED" });
const MERGE_STATE = Object.freeze({ CLEAN: "CLEAN", BEHIND: "BEHIND", DIRTY: "DIRTY" });
const REVIEW_APPROVED = "APPROVED";
const REVIEW_STATE = Object.freeze({ CHANGES_REQUESTED: "CHANGES_REQUESTED", COMMENTED: "COMMENTED" });
const WRITE_PERMISSIONS = ["ADMIN", "MAINTAIN", "WRITE"];

const DEFAULT_MERGE_METHOD = "merge";
const SWEEP_INTERVAL_MS = 60_000;
const SWEEP_LOCK_TIMEOUT_MS = 250;
const DETAIL_FETCH_CONCURRENCY = 4;
const PR_LIST_LIMIT = "200";
const PR_VIEW_FIELDS = "author,reviews,comments";
const PR_LIST_FIELDS = "number,state,reviewDecision,mergeStateStatus,updatedAt,headRefName";

const PR_BODY_ACCEPTANCE_HEADING = "## Acceptance Criteria";
const PR_BODY_GATE_EVIDENCE_HEADING = "## Gate Evidence";

/** Merge-method flag shared by createPr's --auto and the sweep's manual merge fallback. */
function mergeMethodFlag(config) {
  return `--${config.pr?.mergeMethod || DEFAULT_MERGE_METHOD}`;
}

async function mustExec(exec, cmd, args, opts) {
  const res = await exec(cmd, args, opts);
  if (res.code !== 0) throw execError(cmd, args, res);
  return res;
}
...
export function appendNote(root, config, id, text) {
  return mutateBoard(root, config, id, (s) => {
    s.body = board.appendNote(s.body, text);
  });
}

export function appendQuestion(root, config, id, text) {
  return mutateBoard(root, config, id, (s) => {
    s.body = board.appendQuestion(s.body, text);
  });
}
...
export function buildPrBody(story, evidence) {
  const ac = board.readBodySection(story.body, board.SECTIONS.ACCEPTANCE);
  const lines = [
    `Story: \`${story.id}\``,
    "",
    PR_BODY_ACCEPTANCE_HEADING,
    ac || "_none recorded_",
    "",
    PR_BODY_GATE_EVIDENCE_HEADING,
  ];
...
export async function createPr(root, story, { exec = defaultRun } = {}) {
  ...
  await mustExec(exec, "git", ["push", "-u", "origin", branch], { cwd: wt });
  const body = buildPrBody(story, latestEvidence(root, story.id));
  const res = await mustExec(exec, "gh", [
    "pr", "create",
  ...
  const method = mergeMethodFlag(config);
...
export async function integratePrMode(root, story, { exec = defaultRun, now = () => new Date(), diff } = {}) {
  const config = loadConfig(root);
  const paths = diff ?? actualDiff(root, story.id, { exec, base: config.baseBranch });
  const existing = board.prNumber(story);
  let created = null;
  if (existing) {
    await mustExec(exec, "git", ["push", "origin", branchName(story.id)], { cwd: worktreePath(root, story.id) });
  } else {
  ...
  await mutateBoard(root, config, story.id, (s) => {
    const next = board.releaseClaim(reconcileTouches(s, paths), STATUS.IN_REVIEW); // ratified: ownership of an in-review story is its pr record
    delete next.feedback;
    next.pr = existing
      ? { ...next.pr, lastSync: now().toISOString() }
      : { number, lastSync: now().toISOString(), syncAttempts: 0 };
    next.body = board.appendNote(next.body, note);
    return next;
  });
...
export function actionableItems(detail, sinceIso) {
  ...
  for (const r of detail.reviews || []) {
    ...
    const commented = r.state === REVIEW_STATE.COMMENTED && r.body?.trim();
    if (r.state !== REVIEW_STATE.CHANGES_REQUESTED && !commented) continue;
...
export function detectEffects(stories, prs, detailsByNumber, nowIso) {
  ...
  for (const story of stories) {
    const number = board.prNumber(story);
    if (!number) continue;
    const pr = byNumber.get(number);
    if (!pr) continue;
    if (story.status === STATUS.DONE || story.status === STATUS.BLOCKED) continue;
    if (pr.state === PR_STATE.MERGED) {
      effects.push({ type: "merged", id: story.id, number });
      continue;
    }
    if (pr.state === PR_STATE.CLOSED) {
    ...
    if (story.status !== STATUS.IN_REVIEW || isFeedback(story)) continue;
    ...
    if (pr.reviewDecision === REVIEW_APPROVED && pr.mergeStateStatus === MERGE_STATE.CLEAN) {
    ...
    if (pr.mergeStateStatus === MERGE_STATE.BEHIND || pr.mergeStateStatus === MERGE_STATE.DIRTY) {
      effects.push({ type: "drift", id: story.id, number, conflictLikely: pr.mergeStateStatus === MERGE_STATE.DIRTY });
    }
  }
  return effects;
}
...
export async function fetchDetails(numbers, { exec = defaultRun, cwd, cap = DETAIL_FETCH_CONCURRENCY } = {}) {
  const out = new Map();
  const queue = [...numbers];
  const worker = async () => {
    while (queue.length) {
      const n = queue.shift();
      const res = await exec("gh", ["pr", "view", String(n), "--json", PR_VIEW_FIELDS], { cwd });
...
export async function applyEffect(root, effect, deps = {}) {
  const config = loadConfig(root);
  const story = board.findStory(root, config, effect.id);
  switch (effect.type) {
    case "feedback":
...
async function applyFeedback(root, config, story, effect) {
  await mutateBoard(root, config, story.id, (s) => {
    ...
    s.body = board.appendNote(
      s.body,
      `PR #${effect.number} review feedback — claim this story and address via kit:receiving-review:\n${formatFeedback(effect.items)}`,
    );
  });
...
async function applyClosed(root, config, story, effect) {
  await mutateBoard(root, config, story.id, (s) => {
    const next = board.parkStory(
      s,
      `PR #${effect.number} was closed without merging. Reopen it, open a fresh PR from the existing ${branchName(story.id)} branch, or cancel this story?`,
    );
    delete next.feedback;
    return next;
  });
...
async function applyMerged(root, config, story, effect, { exec = defaultRun, teardownFn = teardown } = {}) {
  await mutateBoard(root, config, story.id, (s) => {
    const next = board.closeStory(s);
    next.body = board.appendNote(next.body, `PR #${effect.number} merged on GitHub; story closed by sweep`);
    return next;
  });
  try {
    await teardownFn(root, story.id, { exec });
  } catch (e) {
    await appendNote(root, config, story.id, `worktree teardown failed: ${e.message} — remove ${WORKTREES_DIR}/${story.id} manually`);
  }
...
async function applyApprovedMerge(root, config, story, effect, { exec = defaultRun } = {}) {
  ...
  if (res.code !== 0) {
    await appendNote(
      ...
      config,
      story.id,
      `PR #${effect.number} is approved and mergeable but 'gh pr merge' failed (${failureOutput(res)}) — merge manually`,
    );
...
async function applyDrift(root, config, story, effect, { exec = defaultRun, runGates = runGatesWithEvidence } = {}) {
  const budget = config.budgets.maxFixRoundsPerStory;
  const attempts = Number(story.pr?.syncAttempts ?? 0);
  if (attempts >= budget) {
    await mutateBoard(root, config, story.id, (s) =>
      board.parkStory(s, `PR #${effect.number}: absorbed main ${attempts} times without landing (budget ${budget}). Merge or close it manually?`),
    );
  ...
  await mustExec(exec, "git", ["fetch", "origin", base], { cwd: wt });
  ...
  if (merge.code !== 0) {
    ...
    await mutateBoard(root, config, story.id, (s) => {
      ...
      s.body = board.appendNote(
        s.body,
        `PR #${effect.number}: merging ${base} into the story branch conflicts. Claim this story, resolve the conflict in ${WORKTREES_DIR}/${story.id}, re-run gates, push.`,
      );
    });
  ...
  if (!gate.pass) {
    ...
    await mutateBoard(root, config, story.id, (s) => {
      ...
      s.body = board.appendNote(
        s.body,
        `PR #${effect.number}: absorbed ${base} but gates failed (${failed}). Claim this story, fix, re-gate, push.`,
      );
    });
    return { id: story.id, type: "drift", outcome: "gates-failed" };
  }

  await mustExec(exec, "git", ["push", "origin", branchName(story.id)], { cwd: wt });
...
function readSweepState(root) {
  try {
    return readJson(sweepStatePath(root));
  } catch {
    return {};
  }
}

function writeSweepState(root, state) {
  writeJsonAtomic(sweepStatePath(root), state);
}

export async function sweep(root, opts = {}) {
  const { exec = defaultRun, now = () => new Date(), intervalMs = SWEEP_INTERVAL_MS, force = false, cap = DETAIL_FETCH_CONCURRENCY, lockTimeoutMs = SWEEP_LOCK_TIMEOUT_MS, runGates, teardownFn } = opts;
  const config = loadConfig(root);
...
async function sweepLocked(root, config, { exec, now, intervalMs, force, cap, runGates, teardownFn }) {
  const state = readSweepState(root);
  const nowDate = now();
  ...
  writeSweepState(root, { lastSweep: nowDate.toISOString() });

  const stories = board.loadStories(root, config).filter((s) => board.prNumber(s));
  if (!stories.length) return { swept: true, effects: [] };

  const list = await mustExec(exec, "gh", [
    "pr", "list", "--state", "all", "--limit", PR_LIST_LIMIT,
    "--json", PR_LIST_FIELDS,
  ], { cwd: root });
  ...
  const changed = stories
    .filter((s) => {
      const pr = byNumber.get(board.prNumber(s));
      if (!pr || pr.state !== PR_STATE.OPEN) return false;
      if (s.status !== STATUS.IN_REVIEW || isFeedback(s)) return false;
      return !s.pr.lastSync || Date.parse(pr.updatedAt) > Date.parse(s.pr.lastSync);
    })
    .map((s) => board.prNumber(s));
...
export async function ensurePrRequirements(root, { exec = defaultRun } = {}) {
  ...
  if (perm.code !== 0 || !WRITE_PERMISSIONS.includes(level)) {
    problems.push(
