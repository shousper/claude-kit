...
Loops are per-session — stopping one worker's loop needs its session id (or run this from that worker's own session, whose harness adapter supplies it as `STORY_SESSION_ID`); stopping every loop on the board needs `--all`.

```bash
story loop status --json                # no session: lists every active loop on the board
story loop status --session <id> --json # one worker's loop — capture goal, iteration count, and stalls vs budget BEFORE stopping
...
Report the goal, iteration count, and stalls-vs-budget from the status output. If status reports no active loop (`{active: false}`), say so and continue to the report — the human still wants the picture.
...
- Delete or edit `.agents/shousper-stories/local/loop.*.md` by hand — `story loop stop` owns it.
