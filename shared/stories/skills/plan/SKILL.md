---
name: plan
description: Decomposes a spec or approved design into epics and stories on the story board — sized, dependency-ordered, gated, human-approved, then filed via the story CLI. Use in a project with .agents/shousper-stories/config.json when there is a design doc (often from kit:brainstorming), spec, requirements file, or feature request to break down; it replaces kit:writing-plans in the workflow chain for story-based projects. Also use for "file stories", "break this into stories", "populate the board". DO NOT TRIGGER in projects without .agents/shousper-stories/config.json (use kit:writing-plans), for executing stories (stories:work), or for filing a single discovered-work story mid-loop (plain story create).
---
...
| gates | Omit to accept the type defaults from config; override per story (UI story → add `visual`). Only names defined in `.agents/shousper-stories/config.json` |
...
Board filed. **Your job ends here — report the board (counts + `story board` summary) and STOP.** Execution belongs to separate worker sessions: tell your human partner to open one or more worker sessions and say "complete all stories". Never invoke stories:work, `story claim`, or `story loop start` from this planning session — planning sessions own no loop, and a loop re-prompt naming a story is addressed to the worker session that started that loop, not to you. Do NOT invoke kit:build-flow directly either — it runs per-story inside stories:work.
...
- Act on a loop re-prompt naming a story — those belong to the worker session bound to the loop.
