...
⚠️ **CRITICAL — cd before removal:** You MUST determine the main working tree path and `cd` to it BEFORE running any removal command. Always chain the `cd` and removal in the SAME shell call using `&&`. Never run them as separate shell calls — shell CWD persists between calls, but a deleted CWD will brick every subsequent command.
