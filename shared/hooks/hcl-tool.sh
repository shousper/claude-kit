...
# Sibling of kit_state_dir()'s directory: under the Claude wrapper (which sets
# KIT_STATE_DIR=.../kit/state), this resolves to .../kit/hcl-tool.json — the
# same location as before the KIT_STATE_DIR indirection.
state_file() { printf '%s/hcl-tool.json' "$(dirname "$(kit_state_dir)")"; }
