...
describe("HCL code standard", () => {
  it("code-standards/hcl/CLAUDE.md exists and is substantial", () => {
    const path = resolve(KIT_ROOT, "code-standards/hcl/CLAUDE.md");
    expect(existsSync(path)).toBe(true);
    expect(readFileSync(path, "utf-8").length).toBeGreaterThan(1500);
  });

  it("covers the required sections", () => {
    const text = readFileSync(resolve(KIT_ROOT, "code-standards/hcl/CLAUDE.md"), "utf-8");
  ...
  it("does NOT leak lynx-devops bespoke patterns", () => {
    const text = readFileSync(resolve(KIT_ROOT, "code-standards/hcl/CLAUDE.md"), "utf-8").toLowerCase();
  ...
  it("SKILL.md maps HCL extensions to the standard", () => {
    const text = readFileSync(resolve(KIT_ROOT, "skills/code-standards/SKILL.md"), "utf-8");
  ...
  it("session-start.sh names HCL in the code-standards instruction", () => {
    const text = readFileSync(resolve(KIT_ROOT, "hooks/session-start.sh"), "utf-8");
