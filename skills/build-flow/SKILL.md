---
name: build-flow
description: Executes an implementation plan by driving a background workflow batch-by-batch (implement, then parallel spec + quality review with a staged fix-loop), pausing only when genuinely blocked. Use when you have an implementation plan to execute, are handed or pointed at a written plan file (e.g. a docs/plans/*.md) and asked to implement or execute it, need to build or implement a feature from a plan, or the user says "execute the plan", "implement this plan", "start building", or "build flow". Works the same whether continuing in this session or picking up a plan in a fresh one. DO NOT TRIGGER when there is no plan yet (use kit:brainstorming first) or for ad-hoc unrelated parallel tasks (use kit:parallel-agents).
---
